...
Collections all you to perform actions on groups of resources.

When creating filters, it takes a dictionary.

```python
# Example to print all EC2 instances
import boto3

aws = boto3.session.Session(profile_name="root")
ec2 = aws.resource(service_name="ec2", region_name="us-east-1")

# Example filter used to only show running ec2 instances
f1={"Name": "instance-state-name", "Values":['running']}
# Another filter to only show t2.micro instances
f2={"Name": "instance-type", "Values":['t2.micro']}

# Print every instance in the account with the 'all' submethod
for each in ec2.instances.all():
    print(each)

# You can also use the 'filter' submethod
# Using this filter it only shows running instances that are t2.micro
# It can accept multiple filters separated by a comma
