...
- Responsible for protecting the infrastructure that runs the services.
- Compute, storage, db, networking that runs AWS services
- Regions, AZs, edge locations

Customer Responsibility (Security IN the Cloud)
- The level of responsibility depends on the service they select.
- From most to least control: IaaS, PaaS, SaaS
- IaaS such as EC2: all security and management tasks are customer responsibilities.
- IAM permissions
- OS, network, firewall config
- Client-side data encryption
- Classifying assets
- If there is a way to do it in the console, it is customer responsibility.
...
- EN: enhanced networking; uses single root I/O virtualization (SR-IOV) to provide high performance.
    ...
    - Provides high bandwidth, high packet per second (PPS) and lower inter-instance latencies
    - Two types of EN:
        - Elastic network adapter (ENA) supports up to 100 Gbps
        - Intel 82599 virtual function (VF) interface; supports up to 10 Gbps; used on older instances
...
- EFA: elastic fabric adapter; accelerates high performance computing (HPC) and machine learning applications
    ...
    - Can use OS-Bypass: enables HPC and machine learning applications to bypass the OS kernel and communicate directly with the EFA device.
